...
3. If there are pending messages, check them with `inbox(project_name)`
4. Call `work_board(project)` to see what work is ready
...
- Use `recall_memories("topic")` and `entity_read("topic")` to load knowledge
- Use `secret(action="get", key, project)` to retrieve stored credentials
- Use `workfile_read("component")` to reload working notes from prior sessions