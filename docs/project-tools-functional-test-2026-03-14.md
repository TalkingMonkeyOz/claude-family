...
|---|----------|-------|--------|
| 1 | Low | `get_active_protocol`: version discrepancy between MEMORY.md (v11) and CLAUDE.md (v12) | Query DB to confirm: `SELECT version FROM claude.protocol_versions WHERE is_active = true` |
| 2 | Low | `recall_book_reference`: books migrated to Entity Catalog (49 entities per MEMORY.md); `claude.book_references` likely sparse | Add docstring hint to use `recall_entities(entity_type='book')` instead |
| 3 | Low | `recall_entities`: unknown `entity_type` silently returns empty results instead of validation error | Add entity_type validation against `claude.entity_types` |
| 4 | Low | `VOYAGE_API_KEY` in `mcp.json` is `${VOYAGE_API_KEY}` (unexpanded placeholder); server self-heals via .env fallback but path is fragile | Replace with literal key value in mcp.json |
...
| Item | Status |
|------|--------|
| `DATABASE_URI` | Set correctly in `mcp.json` env block |
| `VOYAGE_API_KEY` | Unexpanded in mcp.json; loaded from .env fallback at startup |
...
**Full details**: See [project-tools-functional-test-detail-2026-03-14.md](project-tools-functional-test-detail-2026-03-14.md)
...
**Created**: 2026-03-14
**Updated**: 2026-03-14
**Location**: C:\Projects\claude-family\docs\project-tools-functional-test-2026-03-14.md
